...
public final class FabricClientTestHelper {
  ...
  public static void waitForScreen(Class<? extends Screen> screenClass) {
    waitFor(
        ...
        client -> client.currentScreen != null && client.currentScreen.getClass() == screenClass);
  }
  ...
  public static void waitForWorldTicks(long ticks) {
    ...
    waitFor(
        "World load",
        client -> client.world != null && !(client.currentScreen instanceof LevelLoadingScreen),
        Duration.ofMinutes(30));
    final long startTicks = submitAndWait(client -> client.world.getTime());
    waitFor(
        "World load",
        client -> Objects.requireNonNull(client.world).getTime() > startTicks + ticks,
  ...
  static void waitFor(String what, Predicate<MinecraftClient> predicate) {
    waitFor(what, predicate, Duration.ofSeconds(10));
  }

  static void waitFor(String what, Predicate<MinecraftClient> predicate, Duration timeout) {
  ...
  static <T> CompletableFuture<T> submit(Function<MinecraftClient, T> function) {
